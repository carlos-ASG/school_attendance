# Delta: student-photos (new capability)

## ADDED Requirements

### Requirement: Optional photo per student managed from the Django admin

Each Student SHALL have an optional photo settable from the Student admin change form. The photo SHALL be stored under `MEDIA_ROOT` and SHALL NOT be displayed anywhere in the teacher panel, the admin changelist, or any import/export flow.
...
#### Scenario: No display outside the admin form

- **WHEN** any teacher-panel page (dashboard, course detail, session pages) renders

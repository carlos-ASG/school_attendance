...
Provide a client-side light/dark theme toggle for the teacher panel: a topbar button that switches the theme without a page load, persists the choice in `localStorage`, applies it before first paint to avoid flashes, and ensures panel styles render legibly under both themes.

TBD: confirm scope details as the capability evolves.

...
Define the attendance JSON API used by the teacher panel's client-side attendance recording: a session-cookie + CSRF authenticated endpoint, scoped to the requesting Teacher, that applies bulk Attendance Record updates (statuses and notes) atomically.

...
def desktop_key(db):
    ...
    DesktopApiKey.objects.create(
        name='estacion-test', prefix=extract_prefix(key), hash=hash_key(key)
    )
    return key
...
def test_validity_extended_ignores_max_in_payload(
    ...
    event = extended_payload(
        payload['credential']['id'], datetime(2027, 6, 30, tzinfo=UTC)
    )

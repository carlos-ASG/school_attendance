...
PHOTO_TOO_LARGE_ERROR = (
    f'La fotografía supera el tamaño máximo de {MAX_PHOTO_SIZE_BYTES // (1024 * 1024)} MB.'
)
...
class StudentPhotoField(forms.ImageField):
    ...
    def to_python(self, data):
        if data is not None and getattr(data, 'size', 0) > MAX_PHOTO_SIZE_BYTES:
            raise ValidationError(self.error_messages['photo_too_large'], code='photo_too_large')
...
class ConvertedPhotoStorage(FileSystemStorage):
    ...
    def _save(self, name: str, content) -> str:
        ...
        return super()._save(target, _normalized_jpeg(content))


def _normalized_jpeg(source) -> ContentFile:
    try:

...
class StudentNotFoundError(ValidationError):
    """El evento issued referencia un estudiante inexistente."""


class CredentialNotFoundError(ValidationError):
    """El evento referencia una credencial inexistente."""


class ExpirationBeyondMaxError(ValidationError):
    """La vigencia pretendida excede la expiración máxima de la credencial."""


class MaxExpirationRequiredError(ValidationError):
...
def _validate_vigencia(
    *,
    ...
    if (
        ...
        raise ExpirationBeyondMaxError(
...
def credential_issue(
    *,
    ...
    if max_expiration is None:
        raise MaxExpirationRequiredError(
    ...
    if student is None:
        raise StudentNotFoundError("Estudiante no encontrado.")
...
def credential_revoke(*, credential_id: UUID, revoked_at: datetime) -> None:
    ...
    try:
        ...
        raise CredentialNotFoundError("Credencial no encontrada.") from None
...
def credential_extend_validity(
    *,
    ...
    extensión. Extender más allá del techo lanza `ExpirationBeyondMaxError`.
    """
    now = timezone.now()
    try:
        credential = Credential.objects.select_for_update().get(pk=credential_id)

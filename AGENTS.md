...
| django-ninja API work: `NinjaAPI`, `api.py`, `@api.get/post/put/patch/delete` operations, `Schema`/`ModelSchema`, `Query`/`Form`/`File` hints, routers, `auth=` / CSRF, OpenAPI docs | `django-ninja` | Use when building or reviewing JSON API endpoints with django-ninja (install via `uv add django-ninja`; references and examples live inside the skill folder). |
| Django business-logic layering (HackSoft styleguide): `services.py` (writes, `transaction.atomic`, `select_for_update`, domain exceptions), `selectors.py` (reads, querysets, `annotate`/`F`), model properties vs selectors, thin views/APIs | `django-style` | Use when writing or reviewing business logic, queries, or mutations in `src/<app>/` (co-load `django-ninja` when the consumer is an API endpoint). |
| Django tests with pytest-django: writing/running/debugging tests in `tests/<app>/test_*.py`, `@pytest.mark.django_db`, fixtures (`client`, `db`, `settings`, `mailoutbox`, `django_assert_num_queries`), test database behavior (`config.test_settings`, in-memory SQLite), pytest invocations | `pytest-django` | Use when writing, running, or debugging tests, or touching `pyproject.toml`'s pytest config / `config/test_settings.py` (co-load `django-style` when testing services/selectors). |
| Alpine.js work: `x-data`, `x-show`/`x-if`, `x-bind` (`:`), `x-on` (`@`) with modifiers, `x-model`, `x-for`, `x-transition`, `x-ref`/`$refs`, `$dispatch`/`$store`/`$watch`, `x-cloak`, `x-teleport`, `Alpine.data()`/`Alpine.store()`, Alpine/HTMX interop | `alpine` | Use when adding or reviewing client-side interactivity in templates (co-load `django-cotton`/`django6-htmx` when those are involved). |
